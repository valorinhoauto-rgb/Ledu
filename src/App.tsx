...
export default function App() {
  ...
  const [userOpenAnswers, setUserOpenAnswers] = useState<Record<number, string>>({});
  ...
  const startQuiz = async (fromUpload = false) => {
    ...
    try {
      if (fromUpload) {
        ...
        data = await geminiService.generateQuiz(subject);
      }
      setQuestions(data);
  ...
  const nextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Finalize and evaluate open answers
      setLoading(true);
      const newEvaluations: Record<number, EvaluationResult> = {};
      
      const openQuestions = questions.filter(q => q.type === "OPEN_ENDED");
      for (const q of openQuestions) {
        const answer = userOpenAnswers[q.id];
        if (answer && answer.trim()) {
          const evalResult = await geminiService.evaluateOpenAnswer(q.question, q.suggestedAnswer || "", answer);
          newEvaluations[q.id] = evalResult;
        }
      }
  ...
  const calculateScore = () => {
    ...
    mcQuestions.forEach((q) => {
      const indexInQuestions = questions.findIndex(quest => quest.id === q.id);
      if (userAnswers[indexInQuestions] === q.correctAnswer) {
        mcCorrect++;
      }
    });
    ...
    const finalScore = Math.round((mcScore + openScore) / 2);
              ...
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
                <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 space-y-3">
                  <h3 className="font-bold text-indigo-900 flex items-center gap-2">
                    <ClipboardCheck className="w-5 h-5" /> Estrutura do Simulado
                  </h3>
                  <ul className="space-y-2 text-sm text-indigo-800/80 font-medium">
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-indigo-400 rounded-full" />
                      10 Questões Objetivas (10 pontos cada)
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-indigo-400 rounded-full" />
                      6 Questões Discursivas (Escolha 3 para responder, 33 pontos cada)
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-indigo-400 rounded-full" />
                      Nota Total: 0 a 100 (Média das duas partes)
                    </li>
                  </ul>
                </div>
                ...
                <div className="space-y-6">
                  <button 
                    ...
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-5 rounded-2xl shadow-lg shadow-indigo-100 transition-all flex items-center justify-center gap-3 group"
                  >
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <h2 className="text-sm font-bold text-indigo-600 uppercase tracking-widest">
                    {questions[currentQuestionIndex].type === "MULTIPLE_CHOICE" ? "Objetiva" : "Discursiva"} - {currentQuestionIndex + 1} de {questions.length}
                  </h2>
                ...
                <div className="text-slate-400 font-medium">
                  {questions[currentQuestionIndex].type === "OPEN_ENDED" && (
                    <span className="text-indigo-600 font-bold text-xs bg-indigo-50 px-3 py-1 rounded-full">
                      Responda pelo menos 3 discursivas
                    </span>
                  )}
                </div>
              </div>
                ...
                {questions[currentQuestionIndex].type === "MULTIPLE_CHOICE" ? (
                  ...
                  <div className="space-y-4">
                    <textarea 
                      ...
                      value={userOpenAnswers[questions[currentQuestionIndex].id] || ""}
                      onChange={(e) => setUserOpenAnswers(prev => ({ ...prev, [questions[currentQuestionIndex].id]: e.target.value }))}
                    />
                    <p className="text-xs text-slate-400 italic">
                      Dica: Tente ser o mais completo possível para uma melhor avaliação.
                    </p>
                  </div>
                )}
              </div>
                ...
                <button 
                  ...
                  disabled={
                    questions[currentQuestionIndex].type === "MULTIPLE_CHOICE" 
                      ...
                      : false // Open ended can be skipped if they answer at least 3
                  }
              ...
              <div className="text-center space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div className="text-3xl font-black text-indigo-600">{calculateScore().mcScore}/100</div>
                    <div className="text-slate-500 font-bold text-sm uppercase tracking-wider">Nota Objetiva</div>
                    <p className="text-xs text-slate-400 mt-1">{calculateScore().mcCorrect} de {calculateScore().mcTotal} acertos</p>
                  </div>
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                  ...
                  <div className="bg-indigo-600 p-6 rounded-3xl shadow-lg shadow-indigo-100 text-white">
                    ...
                    <div className="text-indigo-100 font-bold text-sm uppercase tracking-wider">Média Final</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-10">
                ...
                {questions.map((q, idx) => {
                  if (q.type === "MULTIPLE_CHOICE") {
                    ...
                    const answered = !!userOpenAnswers[q.id];
                    
                    return (
                      <div key={idx} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                        <div className={cn(
                          "px-6 py-3 flex items-center justify-between",
                          answered ? "bg-indigo-50" : "bg-slate-50"
                        )}>
                          <span className={cn(
                            "font-bold text-sm uppercase tracking-wider",
                            answered ? "text-indigo-700" : "text-slate-500"
                          )}>
                            Discursiva {idx + 1}
                          </span>
                          {answered ? (
                            <div className="flex items-center gap-2">
                              <span className="text-indigo-700 font-black text-sm">Nota: {evaluation?.score || 0}/100</span>
                            </div>
                          ) : (
                            <span className="text-slate-400 text-xs font-bold uppercase">Não respondida</span>
                          )}
                        </div>
                        <div className="p-8 space-y-6">
                          <div className={cn(
                          ...
                          {answered ? (
                            <div className="space-y-4">
                              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <p className="text-slate-700 whitespace-pre-wrap">{userOpenAnswers[q.id]}</p>
                              </div>
                              
                              <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                                <h5 className="text-xs font-bold text-emerald-600 uppercase mb-2">Resposta Modelo:</h5>
                                <p className="text-emerald-900 whitespace-pre-wrap">{q.suggestedAnswer}</p>
                              </div>

                              <div className="bg-indigo-600 p-6 rounded-2xl text-white space-y-2">
